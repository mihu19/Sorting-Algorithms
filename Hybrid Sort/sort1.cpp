...
int main(int argc, char* argv[]) {
    ...
    // =========================================================================
    // --- FAST READ: one fread into heap buffer, then manual parse ---
    // =========================================================================
    auto t_read_start = high_resolution_clock::now();

    FILE* in_fp = fopen(argv[1], "rb");
    if (!in_fp) { cerr << "Error: Cannot open input file.\n"; return 1; }

    fseek(in_fp, 0, SEEK_END);
    size_t file_size = (size_t)ftell(in_fp);
    fseek(in_fp, 0, SEEK_SET);

    std::unique_ptr<char[]> in_buf(new char[file_size + 1]);
    size_t bytes_read = fread(in_buf.get(), 1, file_size, in_fp);
    in_buf[bytes_read] = '\0';
    fclose(in_fp);
    ...
    // Branchless-friendly manual integer parser
    auto read_int = [&]() -> int32_t {
        ...
        while (p < end && (unsigned char)(*p - '0') <= 9u)
            val = val * 10u + (unsigned char)(*p++ - '0');
        return neg ? -static_cast<int32_t>(val) : static_cast<int32_t>(val);
    };
    ...
    in_buf.reset(); // free the input buffer as early as possible
    ...
    double read_ms  = duration<double, milli>(t_read_end   - t_read_start).count();
    double sort_ms  = duration<double, milli>(t_sort_end   - t_sort_start).count();
    double total_ms = duration<double, milli>(t_end        - t_read_start).count(); // read → write

    cerr << fixed << setprecision(4)
         << "N: " << N << "\nMethod: " << method << "\nThreads: " << omp_get_max_threads()
         << "\nReading time:          " << read_ms  << " ms\n"
